...
*********************************************************************************
*********************************************************************************
*WHO RESULTS-WEIGHT GAIN BELOW THE NORMAL WEIGHT GAIN
*********************************************************************************
*********************************************************************************
clear
...
*Import dataset
import excel "$projdir/data/raw_data_who_wtgainbelow.xlsx"", sheet("sheet1") firstrow
...
replace logoddsratio=log_or if  logoddsratio==. 
replace  se_logor_manual=se_logor if se_logor_manual==.


*Pooling the effect size based on invidual outcomes.
meta set logoddsratio se_logor_manual, studylabel(study) eslabel(Odds ratio) random(reml) // metaset.
...
gen x_below= string( events_i ) + "/" + string( total_i  )
gen y_within= string( events_w) + "/" + string(total_w)
replace x_below="" if x_below=="./." 
...
levelsof outcome_new, local(levels) 
 foreach l of local levels {
 	local label: label (outcome_new) `l'
		if "`label'" == "hyperbilirubinaemia" {
    local result "Hyperbilirubinaemia"
}
else if "`label'" == "RDS" {
...
meta set logoddsratio se_logor_manual, studylabel(study) eslabel(Odds ratio) random(sjonkman) // metaset.
...
levelsof outcome_new, local(levels) 
 foreach l of local levels {
    ...
    local result "Unknown Condition"
			...
			capture noisily  meta forestplot _id x_below y_within _plot _esci _weight  if outcome_new==`l', random(sjonkman) se(khartung) transform(`"Odds ratio"': exp) subgroup(bmi_new) title("`result'")  nogsig  noosigtest   xlabel( -2 "2" -1 "-1" 0 "0" 1 "1" 2 "2", labsize(medium))  coltitleopts(size(medium)) columnopts(_esci, title("OR (95% CI)") supertitle (""))  columnopts(_weight, title("% Weight") supertitle ("")) columnopts(x_below, title("No. GWG Below""events/total")) columnopts(y_within, title("No. GWG Within""events/total")) itemopts(size(medium)) overallopts(size(medium)) bodyopts(size(medium)) markeropts(msize(small)) insidemarker(msize(small)) esrefline(lpa(dash) lwidth(thin))  nullrefline note(, size(medium)) note(" ") saving(fp_`label', replace)			
 }
...
levelsof outcome_new, local(levels) 
 foreach l of local levels {
 	local label: label (outcome_new) `l'
	display "Outcome is `label'"
capture noisily meta summarize if outcome_new==`l', random(sjonkman) se(khartung) transform(`"Odds ratio"': exp) subgroup(bmi_new)
 }
...
*This analysis was restricted to studies that reported both crude and adjusted odds ratios for the outcomes of interest (LGA, LBW, macrosomia, and SGA) in relation to weight gain below the recommended range. The dataset therefore included only studies providing both crude and adjusted estimates for these outcomes.
*Study=Study name
*Total_i= Total number of participants in the intervention group, from which the number of events (events_i) and non-events are derived.
*Events_i=Number of events of inadequate weight gain
*Total_w=Total number of participants in the control group, from which the number of events (events_i) and non-events are derived.
...
*************************************************************************************************************************************************************
clear
import excel "$projdir/data/raw_data_who_wtgainbelowca.xlsx", sheet("sheet1") firstrow
...
* Check if outcome is lbw or macrosomia – plot without pooling as they only have one study
    if inlist("`label'", "lbw", "macrosomia") {
			capture noisily  meta forestplot _id x_below y_within _plot _esci _weight  if outcome_new==`l', nooverall  transform(`"Odds ratio"': exp) subgroup(crudeoradjusted) nogsig noosigtest title("`result'")    xlabel( -2 "2" -1 "-1" 0 "0" 1 "1" 2 "2", labsize(medium))  coltitleopts(size(medium)) columnopts(_esci, title("OR (95% CI)") supertitle (""))  columnopts(_weight, title("% Weight") supertitle ("")) columnopts(x_below, title("No. GWG Below""events/total")) columnopts(y_within, title("No. GWG Within""events/total")) itemopts(size(medium)) overallopts(size(medium)) bodyopts(size(medium)) markeropts(msize(small)) insidemarker(msize(small)) esrefline(lpa(dash) lwidth(thin))  nullrefline note(, size(medium)) note(" ") saving(fp_`label', replace)			
 }
 
 else {
 	* All other outcomes – plot with random-effects pooling
 capture noisily meta forestplot _id x_below y_within _plot _esci _weight ///
            if outcome_new==`l', ///
            random(reml) se(khartung) transform(`"Odds ratio"': exp) ///
            subgroup(crudeoradjusted) nogsig noosigtest title("`result'") ///
           ///
            xlabel(-2 "2" -1 "-1" 0 "0" 1 "1" 2 "2", labsize(medium)) ///
            coltitleopts(size(medium)) ///
            columnopts(_esci, title("OR (95% CI)") supertitle ("")) ///
            columnopts(_weight, title("% Weight") supertitle ("")) ///
            columnopts(x_below, title("No. GWG Below""events/total")) ///
            columnopts(y_within, title("No. GWG Within""events/total")) ///
            itemopts(size(medium)) overallopts(size(medium)) bodyopts(size(medium)) ///
            markeropts(msize(small)) insidemarker(msize(small)) ///
            esrefline(lpa(dash) lwidth(thin)) nullrefline ///
            note(, size(medium)) note(" ") ///
            saving(fp_`label', replace)
    }
}


**Meta-summary, subgroup by crudeoradjusted odds ratio
levelsof outcome_new, local(levels) 
 foreach l of local levels {
 	local label: label (outcome_new) `l'
	display "Outcome is `label'"
