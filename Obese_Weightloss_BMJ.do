...
*Import dataset
import excel "$projdir/data/raw_data_obese_wtloss.xlsx"", sheet("sheet1") firstrow
...
gen x_weightloss= string( events_i ) + "/" + string( total_i  )
gen y_within= string( events_w) + "/" + string(total_w)
replace x_weightloss="" if x_weightloss=="./."
...
capture noisily  meta forestplot _id x_weightloss y_within _plot _esci _weight  if outcome_new==`l', random(sjonkman) se(khartung) transform(`"Odds ratio"': exp) subgroup(bmi_new) title("`result'")  nogsig noosigtest  xlabel( -2 "2" -1 "-1" 0 "0" 1 "1" 2 "2", labsize(medium))  coltitleopts(size(medium)) columnopts(_esci, title("OR (95% CI)") supertitle (""))  columnopts(_weight, title("% Weight") supertitle ("")) columnopts(x_weightloss, title("No. GWG Weigtloss""events/total")) columnopts(y_within, title("No. GWG Within""events/total")) itemopts(size(medium)) overallopts(size(medium)) bodyopts(size(medium)) markeropts(msize(small)) insidemarker(msize(small)) esrefline(lpa(dash) lwidth(thin))  nullrefline note(, size(medium)) note(" ") saving(fp_`label', replace)			
 }
