...
*********************************************************************************
*********************************************************************************
*OBESE GROUP-WEIGHT GAIN BELOW THE NORMAL WEIGHT GAIN
*********************************************************************************
*********************************************************************************
clear
...
*Import dataset
import excel "$projdir/data/raw_data_obese_wtgainbelow.xlsx"", sheet("sheet1") firstrow
...
gen bmi_new=.
replace bmi_new= 1 if bmi_r==1
replace bmi_new=2 if bmi_r==2
replace bmi_new=3 if bmi_r==3

lab def bmi 1 "Obese 30-34.9" 2 "Obese 35-39.9" 3 "Obese ≥40", replace
lab val bmi_new bmi
 ...
 **Meta-summary (Summarizing the results in the table format)
 ...
 foreach l of local levels {
 	local label: label (outcome_new) `l'
	display "Outcome is `label'"
capture noisily meta summarize if outcome_new==`l', random(jonkman) se(khartung) transform(`"Odds ratio"': exp) subgroup(bmi_new)
 }
save obese_below, replace 